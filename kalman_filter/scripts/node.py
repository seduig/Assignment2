...
class ParticleFilterLocalisationNode(object):
    def __init__(self):
        ...
        self._particle_filter = kalman_filter.pf.PFLocaliser()
...
#	    self.estimatedpose = self._particle_filter.predict_from_odom(odometry, self._particle_filter.estimatedpose.pose.pose)
#	    self._particle_filter.estimatedpose.pose.pose = self.estimatedpose
#	    self.estimatedvel = self._particle_filter.predict_vel(odometry)

            t_filter = self._particle_filter.update_filter(self._latest_scan,odometry)
	    self.estimatedpose = self._particle_filter.estimatedpose.pose.pose
    ...
    def _laser_callback(self, scan):
        ...
        if self._initial_pose_received:
#	    print(self._particle_filter.estimatedpose)
